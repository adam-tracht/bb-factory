...
- Preflight while main is release-shaped: main currently receives only synthesized release commits (commit-trees of factory minus `plans/` plus `dist/`, per scripts/release.sh). After `git fetch origin`, check `git log origin/main -20 --format=%s`. If every commit is a `release:` artifact commit (the two `chore:`/`release:` setup commits at the branch base are known history), skip the generic `git rebase origin/main`: it can only conflict and there is nothing to sync. If any other commit appears on main, main may be live again: run the normal `git rebase origin/main` per the generic protocol. If that rebase conflicts (for example a stray commit landed on a still-release-shaped tree), the existing conflict path applies: abort, file a blocking question noting source landed on the release branch, and stop with `state: blocked`.
