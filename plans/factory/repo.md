...
- Preflight rebase exception, conditional while main stays release-shaped: do not `git rebase origin/main`. Main currently receives only synthesized release commits (commit-trees of factory minus `plans/` plus `dist/`, per scripts/release.sh), so a rebase can only conflict. This is not permanent: after `git fetch origin`, check `git log origin/main -20 --format=%s`. If every commit is a `release:` artifact commit (the two `chore:`/`release:` setup commits at the branch base are known history), proceed without a rebase. If any other commit appears on main, main may have become a live branch again: stop, file a blocking question, and do not claim work until a human decides whether normal rebase preflight resumes.
