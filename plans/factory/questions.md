...
answer: No, 2026-09-15 (via chat): main is not a synthetic release branch indefinitely, so permanently skipping the rebase is wrong. Resolution: repo.md now makes the exception conditional - skip the rebase only while origin/main carries nothing but release-artifact commits, and stop with a blocking question the moment any other commit appears, at which point a human decides whether normal rebase preflight resumes.
