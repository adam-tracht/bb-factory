...
- ✅ **Settlement on structured signals:** Phase 2 records the attached task thread live status and settles only when the BB thread is terminal, the task is in review, the repository commit changed, and the current generation still owns the lease; stale generation or lease evidence is reconciled conservatively.
  Implementation note: the settlement update writes `status: done` and a bounded `factory-settled:<attemptId>` description marker atomically; later reads require that marker, so a card status alone never proves Factory caused settlement.
  Settlement acceptance is scoped to the run: any recorded settlement intent from an attempt of that same run is eligible, but its task id must match the run, its card status at issue must be `in_review`, and its marker must be present in the live description. A marker from another run never attributes settlement.
  Marker matching tolerates whitespace and surrounding text while requiring an exact marker token, so a strict prefix of another attempt id does not match. Settlement marker tokens are excluded from the approval content revision; human description edits remain revision changes.
  `factory-settled:` is a reserved description prefix: any token carrying it is stripped before the approval revision is derived, so two descriptions differing only in such a token hash alike even when a human typed one. Trailing whitespace is likewise outside the revision.
  Known limitation: because Tasks `updateTask` has no expected-revision parameter, a description edit can still land after Factory's final card read and before the settlement mutation. The final read narrows this lost-update window but cannot close it.
...
- ✅ **Factory UI over the Tasks projection**: enabled Work, Questions, Runs, and aggregate views use the existing factory UI over the Tasks-backed snapshot and ledger, with no board redirection; native-card details stay honest, draft-tasks creates backlog cards, and enabled prompts and empty states name Tasks while disabled markdown behavior remains unchanged.
