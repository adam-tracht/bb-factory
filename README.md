...
A [bb](https://getbb.app) plugin that runs unattended coding work on your
repositories. You describe tasks in a Markdown queue inside the repo, an
...
the repository itself, so Git history is the audit trail. The plugin parses
those files, guards every write it makes to them, and keeps only operational
metadata (run intents, ownership leases, idempotency results) in its own
SQLite store. It never mirrors task state into a second database.
...
`ready` when releasing or recovering a claim, never grant it. Anything that
needs a human decision is not a status at all: it becomes a question in
`questions.md` and the entry is marked `blocked-by: Q<n>`.

An entry is eligible when it is `ready`, every `depends_on` target is
satisfied (the named task has completed and moved to `done.md`), and no
open blocking question references it. Eligibility is evaluated per item,
so one blocked entry never stalls the rest of the queue.

### The human gates

Two authorization points are hard requirements, not conventions:

- **Initial `ready`.** The first transition of a queue entry to `ready`
  can only be made through the plugin's native approve control (or by hand
  in the file). Agents cannot perform it. The plugin enforces this with a
  durable one-shot intent bound to the exact entry, revision, and change,
...
make, it records an `assumption` question so the human can reverse it.
Every question carries a `recommended:` line so replying "yes" is enough.
...
1. Replays the recorded result if the request's idempotency key was seen
   before, so retries and double-clicks never double-dispatch.
...
5. Selects a provider: your pinned preference, an explicit Run now
   override, or the configured rotation, skipping providers that are
   ...
   after the usability check.
6. Persists, in one SQLite transaction, the durable run intent (eligible
   items and their authorization provenance), an ownership lease (a unique
   index guarantees one active run per repository), and the dispatch
   attempt.
...
3. *Plan.* Decomposes the task into worker jobs with explicit file
   boundaries, done conditions, and validation commands. Vague acceptance
   criteria become a blocking question, not a guess.
4. *Delegate.* Spawns worker bb threads in the same worktree (models per
   `repo.md`), reviews each job for spec compliance then code quality, and
   fixes or re-briefs before the next job.
5. *Verify.* Runs every `validate:` command plus the repo's standard
   checks itself. Two failed fix rounds means revert the task's files,
   release the entry back to `ready` with a `failed:` note, and record it.
...
7. *Record.* Writes the immutable run record, overwrites `current.md`
   ...
   posts a plain-English report covering what shipped, what did not and
   why, every open question in full, and the merge state of `factory`
   versus `origin/main`.

**Reconciliation.** The plugin watches the worker thread. On completion it
trusts `current.md` only when the file is fresh (modified after the run
started), maps the `state:` line onto the operational run record, updates
counters, and releases the lease. A thread that dies without a fresh state
line is marked `failed-safe` and its provider takes a 6-hour limit. A spawn
that may or may not have landed becomes `reconciliation-required` instead
of being retried blind. Runtime past the configured cap requests
cancellation; the lease releases only after the thread is confirmed
terminated. **Retry** resubmits the original message through bb's retry
path, bounded to three total attempts per run.

## Guarded writes

Every mutation the plugin makes to repository files goes through the
`factory_action` RPC and the same guarded sequence:
...
4. Re-read the target and verify the content landed as intended, then
   record the result.

Resubmitting the same request replays the stored result; a different
payload under the same key returns `idempotency-conflict`. Idempotency keys
have the frozen shape `bbf:v1:<repository>:<operation>:<UUID>`, and the
repository and operation segments must match the request. Writes are
...
a running foreman) follows the same durable-intent pattern: claim the key,
resolve the interaction once, verify the persisted resolution, replay on
retry. An ambiguous or mismatched resolution becomes
`reconciliation-required` rather than a second, different answer.
...
- **Protocol scaffolding**: for a checkout with no `plans/factory/`, one
  guarded action writes the missing files from the bundled templates with
  create-only compare-and-swap (existing content is never overwritten),
  refuses off the `factory` branch, and commits the scaffold on `factory`.
...
- **Questions**: repository questions merged with the foreman's pending BB
  ...
  **Recommend** actions spawn a separate permission-`auto` thread to draft
  an answer or approval; they write nothing to the repository, so the
  human still records the decision.
- **Runs**: operational run history with links to each thread, provider,
  model, duration, and task ids; run detail shows attempts and the linked
  canonical records.
- **Settings**: editable dispatch form (schedule, providers, pause)
  backed by the settings-mutation RPCs, with a plain-English schedule
  preview and next fire times.
...
- **Shell header**: repository switcher, dispatch status chip, running /
  idle badge, pause / resume, and a confirmed **Run now** that optionally
  pins provider, model, and reasoning level for that run.

## Scheduling and dispatch policy

One plugin-owned scheduler evaluates a five-field cron expression in
`server-local` or a named IANA time zone. Semantics:
...
2. Review the derived registration: the wizard fills the repository key
   ...
   Registering the selected checkout directly stays available as the
   advanced path, and when the `factory` branch is already checked out
   elsewhere the wizard offers that checkout instead. Optionally edit the
   friendly display name shown in the Factory UI; the repository key
   remains the technical identity. Registration ends paused.
...
5. Press **Run now** in the header for a supervised first run, or unpause
   dispatch and let the schedule take it. Review `current.md`, `runs/`,
   and `questions.md` from the tabs the next morning.
