...
- Phases 2 and 3 are implemented locally: guarded repository and BB interaction executors consume `pending_action_intents`, the dispatch engine (`src/dispatch/`), scheduler (`src/schedule/`), and lifecycle service (`src/lifecycle/`) are wired through `src/rpc/action-router.ts` and `src/services/action-composition.ts`, and the UI dispatches guarded actions through `ViewContext.onAction` (`src/ui/context.ts`) from the view files under `src/ui/views/`, with the shell header (`src/ui/shell.ts`) owning pause, resume, and run-now and `src/services/settings-mutations.ts` backing the editable settings form and add-repository wizard. Compliance review `thr_k4w2fidc6h` passed after two fixes (stop no longer releases ownership on reconciliation-required runs; queue approval now rejects open blocking questions, unmet dependencies, done items, and repo policy violations). Code-quality review `thr_6cqvkta7rb` passed after fixes for intent persistence on post-claim failure, fail-closed pending-interaction reads, and shared helper consolidation. Focused evidence: 120 tests passing, plus typecheck, lint, and build. Dispatch stays `paused` until the hosting gate closes; Phases 4 and 5 remain open.
...
| P2 action integration, `src/rpc/action-router.ts`, `src/services/action-composition.ts`, `src/ui/views/` | Route the amended action unchanged through the existing source-discriminated request flow and preserve exactly-once behavior by `interactionId` and idempotency key. |
...
| P2 action composition | Source-discriminated routing to the correct guarded executor; Approve queue composes through the repository executor | `src/rpc/action-router.ts`, `src/services/action-composition.ts`, `src/ui/views/`, action integration tests |
