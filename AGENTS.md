...
After any plugin fix that affects running behavior, rebuild and reload the locally installed plugin before reporting done: `pnpm build` then `bb plugin reload bb-factory`. Confirm `bb plugin list` shows the expected version. A pushed release or tag is not enough because Adam's app may be running the path-installed local plugin.
